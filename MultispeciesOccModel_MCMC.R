...
MultispeciesOccModel_MCMC = function(Y, X, V, niter=500, niterInterval=floor(500/4), quiet=FALSE) {
   ...
   ## ... dMvn() is used internally in the bayesm R package.   It is much faster than dmvnorm() in the mvtnorm R package.
    ...
    for (iter in 1:niter) {
        ...
        ## ... draw Z | Y, Beta, Alpha
        ##     (draws are only needed for sites where a species was not detected at least once)
        psi = pnorm(X %*% Beta)
        for (i in 1:n) {
            for (j in 1:m) {
                if (Ysum[i,j]==0) {
                    ...
                    if (psi[i,j] > 0.9999) {
                        Z[i,j] = 1  # analytically, as psi -> 1,  Z | Y, Beta, Alpha  ~ Bernoulli(1)
                    }
                    else {
                        ## probNotDetected = prod(1-pvec)
                        probNotDetected = exp(sum(log(1.0-pvec)))
                        Z[i,j] = rbinom(1, size=1, prob=psi[i,j]*probNotDetected / ( psi[i,j]*probNotDetected + 1-psi[i,j]) )
                    }
                }
            }
        }
